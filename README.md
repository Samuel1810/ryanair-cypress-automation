# Ryanair — Cypress E2E Automation Tests (Lisbon → Madrid)
...
> **Route:** **Lisbon → Madrid** (fixed by design)
...
- **Approach:** Validate one complete “happy path” booking journey up to a stable checkpoint (before sensitive or highly unstable steps).
...
### ✅ Positive
- **TC001 — Successful booking flow (Lisbon → Madrid)**
  - Search flights → select departure/return → choose fare → continue as guest → fill passenger details → handle seats/bags/extras → reach login request checkpoint.
...
✅ `cypress.env.json` is intentionally ignored by Git to avoid committing machine-specific configuration.
...
├─ e2e/
│  └─ purchase/
│     └─ TC001-successful-purchase.cy.js
├─ pages/
│  ├─ HomePage.js
│  ├─ BookingDetails.js
│  ├─ FlightsPage.js
